...
void toString(int coef, int exp) { // coefficient (계수), exponent(차수)를 항으로 출력. 
...
void addTerm(int a[][10], int coef, int exp){ // 다항식에 항들 추가 
...
void sorting(int a[][10]){ // bubble sorting -> 내림차순
...
void displayPoly(int a[][10]){ // 다항식 display
    int bin = 0;
    for(int i = 0; i < sizeof(a[0])/sizeof(int); i++){
...
void addPoly(int a[][10], int b[][10]){  // 두 다항식의 합을 연산

    int poly[][10] = {NULL, }; // 배열 하나 추가

    for(int i = 0; i < sizeof(a[0])/sizeof(int); i++){ // poly 라는 배열에 두 다항식을 차례로 기입 
        if(a[0][i] != 0 && a[1][i] != 0){
    ...
    fix(poly); // 같은 차수의 항이 있으면 서로 더해 하나로 만듦
    displayPoly(poly); 
...
void multiPoly(int a[][10], int b[][10]){ // 두 다항식의 곱을 연산 

    int poly[][10] = {NULL, };
    for(int i = 0; i < sizeof(a[0])/sizeof(int); i++){ // 첫 다항식을 기준으로 항을 하나씩 이동시키면서 두번째 다항식과 교차하며 모든 항 계산
        for(int j = 0; j < sizeof(a[0])/sizeof(int); j++){
            if((a[0][i]==0) || (a[0][j]==0)|| (b[0][i] == 0)||(b[0][j] == 0)) break;

            poly[0][multiCount] = a[0][i] * b[0][j]; // 계수는 곱셈으로 
            poly[1][multiCount] = a[1][i] + b[1][j]; // 차수는 덧셈으로
            multiCount++;
        }
